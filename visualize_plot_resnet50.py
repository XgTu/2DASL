...
from benchmark_0resnet50 import extract_param
...
def aflw2000():
    arch = 'resnet50'
    device_ids = [0]
    checkpoint_fp = './train_debug_resnet/2_wpdc_adverse_1local_exp_projLoss_18pts_mask_shufTrn/_checkpoint_epoch_9.pth.tar'

    params = extract_param(
        checkpoint_fp=checkpoint_fp,
        root='test.data/umd_tst_crop',
        filelists='test.data/umd_tst_crop.txt',
        arch=arch,
        device_ids=device_ids,
        batch_size=128)
    _dump('res/params_aflw2000_xgtu.npy', params)
...
def draw_landmarks():
    ...
    params = _load('res/params_aflw2000_xgtu.npy')
...
#        lms = convert_to_ori(lms, i)
        ...
        plt.savefig('res/3dLandmarks_proj_resnet50/' + img_fp.split('/')[-1])
